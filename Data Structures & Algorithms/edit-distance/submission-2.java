class Solution {
    ...
    public int knapsack(int i,int j,String s1, String s2){
        ...
        if(s1.charAt(i)==s2.charAt(j)){
            dp[i][j] = knapsack(i+1,j+1,s1,s2);
        }else{
            dp[i][j] = Math.min(knapsack(i,j+1,s1,s2),knapsack(i+1,j,s1,s2));//insert,delete
            dp[i][j] = Math.min(dp[i][j],knapsack(i+1,j+1,s1,s2));//replace
            dp[i][j]+=1;
        }
    return dp[i][j];
    }
